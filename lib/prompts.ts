...
2. 괄호로 영어 표현 추가 절대 금지:
   - (Real & Experience), (Fact), (Interpretation) 등 영어 괄호 표현 절대 금지
   - 어떤 문장 뒤에도 (영어 설명) 형태로 추가 설명 금지
   - 모든 내용은 순수 한글로만 작성
   - 괄호 안에 영어 단어 넣지 않기

3. 사용할 형식:
   - 섹션 구분: 빈 줄 3개로 섹션 구분 (네이버 블로그에서 구분선 추가 용이)
   - 섹션 제목: 【제목】 형식
   - 목록: 1. 2. 3. 또는 • 사용
   - 정보: 세로 나열

3. 네이버 블로그 에디터에 바로 붙여넣기 가능한 형태로 작성
   - 각 섹션 사이에 빈 줄 3개를 넣어서 사용자가 구분선을 쉽게 추가할 수 있게 함

4. 이미지 위치 표시: [이미지: 구체적인 사진 설명] 형식으로 작성
...
위 요청에 맞게 블로그 글을 수정해주세요.

중요 규칙:
1. ** 별표 두개(볼드 마크다운) 절대 사용 금지
2. (Real & Experience), (Fact), (Interpretation) 등 괄호 영어 표현 절대 금지
3. 마크다운 문법 절대 사용 금지 (# ** | - 등)
4. 이모지 절대 사용 금지
5. ━━━ 같은 특수문자 구분선 절대 사용 금지
6. 【】 형식의 섹션 제목과 빈 줄 3개로 섹션 구분
7. 네이버 블로그에 바로 붙여넣기 가능한 형태로 작성
8. 전체 글을 다시 작성해서 출력하세요
9. FIRE 공식을 적용하되 괄호 레이블이나 영어 표현은 절대 사용 금지
10. 모든 내용은 순수 한글로만 작성`;
}
