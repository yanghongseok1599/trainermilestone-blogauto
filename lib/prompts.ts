...
const STYLE_GUIDELINES: Record<WritingStyle, string> = {
  storytelling: `
### 글쓰기 스타일: 스토리텔링형
- 실제 경험을 기반으로 한 이야기 구조로 작성
- 첫 방문부터 현재까지의 경험을 시간 순서로 전개
- "처음에는 ~했는데, ~하고 나니, 지금은 ~" 형식
- 감정 변화와 구체적인 에피소드 중심
- 독자가 마치 직접 경험하는 듯한 몰입감 제공
- 결론에서 "그래서 추천하는 이유는~"으로 자연스럽게 연결`,

  informative: `
### 글쓰기 스타일: 정보 제공형
- 핵심 정보를 조목조목 정리하여 전달
- 객관적인 데이터와 수치 중심
- 비교 가능한 정보(가격, 시간, 규모 등)를 명확히 제시
- "~입니다", "~됩니다" 체로 정보 전달
- 불필요한 감정 표현 최소화
- FAQ 형식을 적극 활용하여 궁금증 해소`,

  comparison: `
### 글쓰기 스타일: 비교 분석형
- 유사 업체/옵션과의 비교 관점에서 작성
- "다른 곳과 비교했을 때~", "~와 달리" 표현 활용
- 장단점을 균형있게 제시하되 해당 업체의 강점 부각
- 가격 대비 가치, 시설 비교, 서비스 비교 등 구체적 비교
- 표/리스트 형식으로 한눈에 비교 가능하게 정리
- "결론적으로 ~를 원한다면 이곳이 적합"으로 마무리`,

  review: `
### 글쓰기 스타일: 생생 후기형
- 실제 이용 경험을 생생하게 전달
- 오감을 활용한 구체적 묘사 (보이는 것, 느낀 것, 들은 것)
- "직접 가보니~", "실제로 이용해보니~" 표현 활용
- 기대했던 것 vs 실제 경험의 차이점 언급
- 사진 설명을 풍부하게 (언제, 어디서, 무엇을 찍었는지)
- 솔직한 장단점 언급으로 신뢰성 확보`,

  guide: `
### 글쓰기 스타일: 가이드/안내형
- 처음 방문하는 사람을 위한 완벽 가이드
- 단계별 안내 (예약 → 방문 → 이용 → 마무리)
- "~하는 방법", "~하면 됩니다" 형식의 실용적 안내
- 꿀팁, 주의사항을 별도 섹션으로 강조
- 체크리스트, 준비물 목록 등 실용 정보 포함
- "첫 방문 전 알아두면 좋은 점"으로 시작`,
};
...
const INTENT_OPENINGS: Record<SearchIntent, string[]> = {
  information: [
    '이 글에서 {keyword}에 대한 모든 궁금증을 해결해 드리겠습니다.',
    '{keyword} 관련 정보, 핵심만 정리했습니다. 3분이면 충분합니다.',
    '결론부터 말씀드리면, {keyword}은(는) 이렇습니다.',
    '{keyword}가 처음이신 분들을 위해 알기 쉽게 정리했습니다.',
  ],
  transaction: [
    '{businessName}의 가격과 혜택, 지금 바로 확인해 보세요.',
    '이 글을 끝까지 읽으시면 {businessName}의 숨은 할인 정보까지 알게 됩니다.',
    '{keyword} 등록을 고민 중이시라면, 가격 비교부터 시작하세요.',
    '지금 등록하면 받을 수 있는 특별 혜택을 정리했습니다.',
  ],
  navigation: [
    '{businessName} 찾아가는 방법, 주차 정보까지 완벽 정리했습니다.',
    '처음 방문하시는 분들을 위한 상세 길 안내입니다.',
    '{businessName}까지 가장 빠르게 가는 방법을 알려드립니다.',
    '대중교통, 자차 이용 모두 상세히 안내해 드리겠습니다.',
  ],
  location: [
    '{businessName}을(를) 직접 다녀온 솔직한 후기입니다.',
    '{keyword}를 찾고 계신다면, 이 글이 도움이 될 겁니다.',
    '3개월째 {businessName}을(를) 다니며 느낀 점을 정리했습니다.',
    '{keyword} 어디가 좋을지 고민이시라면, 제 경험을 참고해 보세요.',
  ],
};
