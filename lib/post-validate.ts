...
const FORBIDDEN_WORD_REPLACEMENTS: [RegExp, string][] = [
  ...
  // ── 피트니스 특화 ──
  [/살\s*빠지는\s*운동/g, '체중 관리에 도움되는 운동'],
  [/살\s*빠지는/g, '체중 관리에 도움되는'],
  [/다이어트\s*성공/g, '건강한 식단 관리'],
  [/PT\s*효과\s*보장/g, '꾸준한 운동의 장점'],
  [/효과\s*보장/g, '꾸준히 하면 도움'],
  [/허리\s*통증\s*치료/g, '허리 건강에 좋은 스트레칭'],
  [/통증\s*치료/g, '건강 관리'],
  [/통증\s*완화/g, '편안한 컨디션 관리'],

  // ── 2. 광고/상업성 표현 ──
  [/할인(가|율|폭|된|받|행사|중|합|해|하|을|이|은|는|의|에)/g, '혜택$1'],
  [/할인/g, '혜택'],
  [/무료\s*체험/g, '부담 없는 첫 체험'],
  [/무료/g, '부담 없이'],
  [/공짜/g, '부담 없이'],
  [/0원/g, '부담 없는 가격'],
  [/최저가/g, '합리적인 가격'],
...
function checkForbiddenWords(text: string): ValidationIssue[] {
  ...
  const forbiddenWords = [
    ...
    '시술', '부작용', '안전 보장', '놀라운 효과', '기적',
    // 광고/상업
    '할인', '무료', '공짜', '0원', '최저가', '파격', '특가', '세일', '덤핑',
    '경품', '사은품', '선착순', '한정 수량', '마감 임박', '구매 링크',
    // 과장
    '1등', '1위', '최초', '유일무이', '획기적', '혁신적',
    '100% 효과', '효과 보장', '완벽하게 회복',
    // 피트니스 특화
    '살 빠지는', '다이어트 성공', 'PT 효과 보장',
    // 금융
    '고수익', '원금 보장', '수익 보장',
  ];
